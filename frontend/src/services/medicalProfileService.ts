...
class MedicalProfileService {
  ...
  async getPatientProfile(patientId: number): Promise<MedicalProfileResponse> {
    const token = localStorage.getItem('token');
    console.log('🔐 Getting patient profile - token exists:', !!token, 'token length:', token?.length);
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || response.status === 401) {
        console.log(`No medical profile found for patient ${patientId} (status: ${response.status}) - returning empty profile`);
        return {
          id: null,
          userId: patientId,
          bloodType: null,
